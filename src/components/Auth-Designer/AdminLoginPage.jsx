...
import { useNavigate } from 'react-router-dom'; // ✅ Fixed - removed extra m';
...
export const AdminLoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Use consistent token storage (remove duplicate setters)
      if (response.token) {
        authService.setAuthToken(response.token); // Only use main setter
      }

      if (response.admin) {
        authService.setUser(response.admin); // Only use main setter
      ...
      navigate('/admin/dashboard', {
        state: {
          message: response.message || `Welcome back, Admin ${response.admin?.email}!`,
          user: response.admin
        }
      });
