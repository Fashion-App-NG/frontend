...
const ProductCard = ({ 
  ...
  const { addToCart, isInCart } = useCart(); // ✅ Re-add cart functionality
  ...
  useEffect(() => {
    const loadVendor = async () => {
      if (!showVendorInfo || !product.vendorId) return;
      
      try {
        const response = await vendorService.getVendorById(product.vendorId);
        setVendor(response);
      } catch (error) {
        console.error('Failed to load vendor:', error);
        // Set fallback vendor info
        setVendor({ storeName: 'Fashion Store' });
      }
    };

    loadVendor();
  }, [product.vendorId, showVendorInfo]);
      ...
      <div className="p-4">
        ...
        {showVendorInfo && (
          <div className="flex items-center text-xs text-gray-600">
            ...
            <span>By: {vendor?.storeName || 'Fashion Store'}</span>
          </div>
        )}
